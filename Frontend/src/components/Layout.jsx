...
export const Layout = ({user, onLogout}) => {
    ...
    const fetchTasks = useCallback(async () => {
        ...
        try {
            ...
            const {data} = await axios.get("https://task-manager-api-one-beta.vercel.app/api/tasks/gp", {
    ...
    const StatsCard = ({title, value, icon}) => {
        return (
            <div className="p-2 sm:p-2 rounded-xl bg-white shadow-sm border-purple-100 hover:shadow-md transition-all duration-300 hover:border-purple-100 group">
    ...
    if (loading) {
        return (
            <div className="min-h-screen flex items-center justify-center ">
                <div className="w-12 h-12 animate-spin rounded-full border-t-2 borde-b-2 border-purple-500" />
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50  p-6">
                <div className="bg-red-50 text-red-600 p-4 rounded-xl border border-red-100 max-w-md">
                    ...
                    <button className="mt-4 py-2 px-4 bg-red-100 text-red-700 rounded-lg text-sm font-medium hover:bg-red-200 transition-colors cursor-pointer">
                        Try Again
                    </button>
                </div>
            </div>
            ...
            <div className="ml-0 xl:ml-64 lg:ml-64 md:ml-16 pt-16 p-3 sm:p-4 md:p-4 transition-all duration-300">
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6 ">
                    <div className="xl:col-span-2 space-y-2 sm:space-y-4 ">
                        <Outlet context={{tasks, refreshTasks: fetchTasks, onLogout}}></Outlet>
                    </div>
                    ...
                    <div className="xl:col-span-1 space-y-4 sm:space-y-6 ">
                        <div className=" bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100">
                            <h3 className="text-base sm:text-lg font-semibold mb-3 sm:mb-4 text-gray-800 flex items-center gap-2 ">
                                <TrendingUp className="w-4 h-4 text-purple-500 sm:w-5 sm:h-5" />
                                Task Statistics
                            </h3>
                            ...
                            <hr className="my-3 sm:my-4 border-r-purple-200 "></hr>

                            <div className="space-y-2 sm:space-y-3 ">
                                <div className="flex justify-between items-center text-gray-700">
                                    <span className="text-xs sm:text-sm flex items-center gap-1.5 font-medium">
                                        <Circle className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-purple-500 fill-purple-500"></Circle>
                                        Task Progress
                                    </span>
                        ...
                        {/* RECENT ACTIVITIES */}
                        <div className="bg-white rounded-xl sm:p-5 shadow-sm border border-purple-100">
                            <h3 className="text-base sm:text-lg font-semibold mb-3 sm:mb-4 text-gray-800 flex items-center gap-2">
                                <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-purple-500"></Clock>
                                Recent Activity
                            </h3>
                            ...
                            <div className="space-y-2 sm:space-y-3 ">
                                ...
                                {tasks.length == 0 && (
                                    <div className="text-center py-4 sm:py-6 px-2">
                                        <div className="w-12 h-12 sm:w-16 sm:h-16 mx-auto flex items-center justify-center sm:mb-4 rounded-full bg-purple-100">
                                            <Clock className="w-6 h-6 sm:w-8 sm:h-8 text-purple-500"></Clock>
                                        </div>
